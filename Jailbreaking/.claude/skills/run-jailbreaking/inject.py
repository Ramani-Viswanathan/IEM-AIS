...
Given ANY URL (no target-profile file, no hardcoded target), this:
  ...
  3. If an LLM interface is found, builds 8 attack prompts
     (prompt_generator.build_prompts) grounded in OWASP LLM01:2026's 8
     "Common Examples of Risk" (p.12-13) and matching "Example Attack
     Scenarios" (p.15-17), contextualized to THIS site's real, discovered
     objective.
...
def _run_prompt_entry(origin, endpoint, resolved_extra, p):
    ...
    run_full's 8-prompt batch and run_one's single-row test so both take
...
def run_one(url, risk_id, prompt_text, followup_prompt=None, followup_same_session=False, extra_fields=None):
    ...
    the standard 8. risk_id/citation/meaning/remediation are looked up
...
def run_full(url, extra_fields=None):
    ...
    evidence = {
        ...
        "target_url": url,
        "timestamp": timestamp,
        "standard_citation": "OWASP GenAI LLM Top 10 2026, LLM01:2026 Prompt Injection -- 'Types of Prompt Injection' (p.11), 'Common Examples of Risk' (p.12-13), 'Example Attack Scenarios' (p.15-17)",
        "site_profile": profile,
    }
