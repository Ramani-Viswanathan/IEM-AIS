...
Given ANY URL (no target-profile file, no hardcoded target), this:
  ...
  3. If an LLM interface is found, builds 7 attack prompts
     (prompt_generator.build_prompts) grounded in OWASP LLM10:2026's 7
     "Common Examples of Risk" (p.55-56) and matching "Example Attack
     Scenarios" (p.57), contextualized to THIS site's real, discovered
     objective.
...
(a MATCH = a finding, not a refusal). See classify_output() below; it does
not reuse REFUSAL_MARKERS at all, by design.
...
# unescaped, executable-looking payload" -- the opposite question from the
# other two skills' REFUSAL_MARKERS. A MATCH here is the bad outcome.
...
def classify_output(risk_id, response_text):
    ...
    not a refusal. See DANGEROUS_PATTERNS and the module docstring."""
...
def _flag_duplicate_responses(results):
    ...
    see before. Overlays onto (doesn't discard) the pattern-match verdict."""
...
def _run_prompt_entry(origin, endpoint, resolved_extra, p):
    """Sends one prompt dict (risk_id, prompt) and returns the full result
    entry. Shared by run_full's 7-prompt batch and run_one's single-row
    test so both take the exact same real path -- no separate 'quick
    test' code path. No risk in this skill has prompt=None (see
    prompt_generator's module docstring), but the None-guard is kept for
    structural parity with the other two skills' _run_prompt_entry."""
    if p["prompt"] is None:
    ...
    entry["verdict"] = "ERROR" if outcome.get("error") else classify_output(p["risk_id"], outcome.get("response_text"))
    ...
    if p.get("followup_prompt"):
        ...
        entry["followup_verdict"] = "ERROR" if followup_outcome.get("error") else classify_output(p["risk_id"], followup_outcome.get("response_text"))
...
def run_full(url, extra_fields=None):
    ...
    evidence = {
        ...
        "target_url": url,
        "timestamp": timestamp,
        "standard_citation": "OWASP GenAI LLM Top 10 2026, LLM10:2026 Improper Output Handling -- 'Description' (p.55), 'Common Examples of Risk' (p.55-56), 'Example Attack Scenarios' (p.57)",
        "site_profile": profile,
    }
