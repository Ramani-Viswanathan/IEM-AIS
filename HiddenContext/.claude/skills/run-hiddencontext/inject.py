#!/usr/bin/env python3
"""
Hidden Context Exposure test-case orchestrator -- IEM-AIS, generic, URL-driven.
...
case. This file owns only what's specific to LLM08:2026 Hidden Context
Exposure: REFUSAL_MARKERS, classify(), and this skill's OWASP
citation/probe name.
...
Given ANY URL (no target-profile file, no hardcoded target):
  ...
  3. If found, builds 5 attack prompts (prompt_generator.build_prompts)
     grounded in OWASP LLM08:2026's 5 "Common Examples of Risk" (p.47-48)
     and matching "Example Attack Scenarios" (p.49) where one exists.
  4. Sends every prompt (all 5 -- every risk here has a real chat-message
     channel) to the real, live endpoint and records the real response.
...
PROBE_NAME = "hidden_context_generic"
STANDARD_CITATION = (
    "OWASP GenAI LLM Top 10 2026, LLM08:2026 Hidden Context Exposure -- "
    "'Description' (p.46), 'Common Examples of Risk' (p.47-48), "
    "'Example Attack Scenarios' (p.49)"
)
...
def main():
    ...
    out_path = out_dir / f"hidden_context_generic_{datetime.now().strftime('%y%m%d_%H%M%S')}.json"
    out_path.write_text(json.dumps(evidence, indent=2), encoding="utf-8")

    print(f"Hidden Context Exposure probe complete for {args.url}")
